...
import { isMappingCheckResult, isMappingEvent, MAPPING_CHECK, MAPPING_CHECK_RESULT, MAPPING_EVENT, MAPPING_INSTALL, MAPPING_REVOKE, sameMapping } from '../../contracts/mapping.ts';
import type { MappingCheck, MappingEvent, MappingIdentity, MappingSelection } from '../../contracts/mapping.ts';
...
export async function createPreviewMapping(outputRoot: string, preview: ProjectPreview,
  ...
  return {
    ...
    // A point-in-time identity check, not a write lease. Future draft commands must
    // validate and mutate synchronously in the registry, then revalidate source in Main.
