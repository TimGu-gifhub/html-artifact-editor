...
import type { SourceIdentity, SourceIndex } from '../../core/parser/source-index.ts';
...
export function parseSource(outputRoot: string, bytes: Uint8Array, identity: SourceIdentity,
  signal: AbortSignal = new AbortController().signal): Promise<SourceIndex> {
  ...
  return new Promise((resolveSource, reject) => {
    const worker = new Worker(resolve(outputRoot, 'parser-worker/index.cjs'), {
      workerData: { bytes: new Uint8Array(bytes), identity },
    ...
    const finish = (source?: SourceIndex, error?: string): void => {
      ...
      void worker.terminate().then(() => {
        ...
        resolveSource(Object.freeze({ ...source, get bytes() { return new Uint8Array(snapshot); } }));
