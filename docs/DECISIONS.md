...
## ADR-017：模式切换重建独立文档，私有干净历史通过新源码证明延续

**2026-09-10 HAE-008 采用。** 同一个 Main Workspace 支持静态校稿与脚本只读文档两种明确类型。只读文档无映射、输入、候选或 writer；可信 IPC 只接受 documentId/stateRevision 和两个 mode 值。切换先排空输入并处理取消、放弃或独占另存，再准备运行本地脚本的隔离视图。两个方向沿用原目录身份，试挂载前后验证源版本，经离开屏障确认后发布新文档并清理旧视图。

干净状态的完整历史只在 Main 保留；返回校稿时重建源码索引和历史，重新证明保存为空的 Text，不接受动态 DOM 或旧 offset。先前有编辑时必须核验最新干净检查点；从未编辑的 revision=1 不入持久化队列。明确放弃或另存后不携带脏历史。外部文件变化拒绝旧历史迁移，明确重新打开可接受新的来源。

代价是切换会重载页面、丢弃脚本运行状态并分配新的干净持久化序列；现有跨会话记录仍保留，不引入自动清理。收益是只读模式在类型和 Main 权限上均没有写路径，同时可继续已核验的撤销/重做。版本、退役、Worker 或视图清理不确定时保留已有证据，不能借模式切换绕过恢复。合同与真实产品回归见 [模式合同](MODE_SWITCH.md) 和 [HAE-008 第三阶段](implementation/HAE-008.md)。
