...
HAE-010 第四阶段已在 Main 实现显式备份恢复：验证完整记录及备份，重新固定当前授权文件版本，先备份当前文件，再用同一 Windows 事务替换。恢复记录使用 v2 intent 的 restoreOf 关联来源事务，普通 v1 intent 继续读取。恢复整份备份的字节与恢复尚未保存的逻辑编辑意图是两个动作；目前未实现后者、恢复 UI 或遗留锁处理。合同与平台边界见 [保存与备份恢复](SAVE_PREPARATION.md)。
