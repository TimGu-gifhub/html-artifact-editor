...
HAE-011 第一阶段已实现 [逻辑草稿检查点](DRAFT_CHECKPOINTS.md) 的纯核心重建及 Main 私有存储。恢复输入只有逻辑 Text 意图；从匹配的原始基线重新解析节点和上下文，生成新范围/编码，核对完整候选 hash。当前授权文件身份与 hash 不符即拒绝；与保存 committed 的精确新版本对应时拒绝重复应用。两类记录共用目录、锁和配额，但不自动选取最新点、回退旧点或清除记录。每次 Apply、恢复窗口、历史和遗留锁处理仍待接入。
