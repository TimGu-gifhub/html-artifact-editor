...
版本：schema 1 设计基线。HAE-003/004 已实现静态索引与纯字节 Patch 候选；HAE-005 验证 Main 草稿和独占新文件另存；HAE-010 验证 Windows 覆盖事务及窗口会话的新基线。HAE-011 已连接 Main 历史命令与 v2 完整检查点恢复；HAE-009 已接入产品工作台，HAE-012 已补充产品强杀恢复与跨保存历史的自动证据。遗留故障处置仍待实现；下文各阶段段落保留当时的实验边界。
...
HAE-011 的内部 SourceIndex 可带 lineage，用于重建已保存后消失的历史 Text。它只保存最初来源字节和逻辑值，没有外部写入位置。每次编译必须重新核验当前解析树和整份来源的字面分段；仅经此证明的空目标允许零长度范围，并产生新的源身份。普通解析节点和 UI/页面命令不能声明这种范围，完整规则见 [逻辑历史](HISTORY.md)。
...
HAE-011 第六阶段完成第 5 步的数据与保存确认路径：[源码 Diff](SOURCE_DIFF.md) 使用同一冻结基线/候选的真实 UTF-8 切片，完整呈现实体和换行等词法变化。范围、内容和候选 hash 均由 Main 复核；从 Diff 发起的 Save 携带所显示的修订/hash，过期确认在事务开始前拒绝。保存仍通过 HAE-010 备份、版本冲突和显式提交，新基线重新建索引；产品 Diff 面板与窗口内跨保存反向编辑尚未实现。

目标节点改为空字符串后，内存编辑会话保留该 Text 对象；保存重解析时空节点可能不再存在。清空选择并保留逻辑历史，恢复该节点必须重新生成经验证的相反操作，不能把旧 nodeId 强行复用。

## 7. 保存事务

HAE-005 的 M1 简化另存已实现为独占创建同目录新 HTML：冻结当前候选，选择目标后用 O_EXCL 打开并保留句柄，写入/flush/回读 hash，核对目录链与叶子身份。取消零写入；已有文件一律拒绝；创建后的任何异常标记 unknown 并保留现场。另存不清空原入口草稿或移动保存点，不复制资源或自动切换入口。覆盖事务是独立的 HAE-010 路径，不能用新文件实验代替。

单文件事务设计，不承诺跨多个文件原子性。普通本地磁盘是保证验证范围，网络盘和云同步目录先作为条件支持。

HAE-010 第一阶段验证了私有准备；第二阶段将同目录候选、Windows ReplaceFileW、结果回读和 committed 记录接到 Main 显式 commit；第三阶段经可信 Workspace save 命令重建保存基线与映射。文件版本在打开时固定，保存中的草稿互斥；新文档必须再次匹配提交版本，旧节点身份不复用。`prepared` 不代表 HTML 已保存；进程/日志异常或重建失败保留证据并禁止盲目重试。正常应用、跨保存的撤销历史和恢复 UI 仍未接入；完整执行范围和平台边界见 [保存事务](SAVE_PREPARATION.md)。
...
HAE-010 第四阶段已在 Main 实现显式备份恢复：验证完整记录及备份，重新固定当前授权文件版本，先备份当前文件，再用同一 Windows 事务替换。恢复记录使用 v2 intent 的 restoreOf 关联来源事务，普通 v1 intent 继续读取。恢复整份备份的字节与恢复尚未保存的逻辑编辑意图是两个动作；合同与平台边界见 [保存与备份恢复](SAVE_PREPARATION.md)。

HAE-011 已实现 [逻辑草稿检查点](DRAFT_CHECKPOINTS.md) 的纯核心重建、Main 私有存储和可选 Apply 队列。v1 恢复输入仅为逻辑 Text 意图，v2 另有完整历史链和最初来源；从匹配的原始基线重新解析节点和上下文，生成新范围/编码，核对完整候选 hash。当前授权文件身份与 hash 不符即拒绝；与保存 committed 的精确新版本对应时拒绝重复应用。最新点按会话修订选择，不能越过较新不完整或归零点；明确结束标记阻止旧会话恢复和继续写入。窗口离开已在输入冻结、写入排空及视图试挂载后核验结束标记；失败/未知保留原草稿及源证据并阻止继续操作。两类记录共用目录、锁和配额，不自动删除证据。可信 Workspace 已能在重新授权、核验最新记录和源版本后，将候选安装到新 Preview 映射；恢复不写 HTML，不重复持久化同一修订。产品恢复/历史控件、故障恢复操作、清理和遗留锁处理仍待接入。
...
HAE-011 第七阶段已实现 [纯核心历史与来源重建](HISTORY.md)，包含 prepare/commit、独立保存点、分支与私有逻辑记录校验；真实 Worker、Windows 事务和 Chromium 重开分别验证。第八阶段将新来源证明接入隔离 Preview：先核对实际 DOM，按明确的空 Text 安装合同创建缺席对象，再按映射版本与旧值确认单 Text 历史操作。第九阶段将上述证明接入 Main 文档、InputController、可信 Workspace history 命令和 v2 完整历史检查点；确认后才推进并持久化历史，干净点恢复不回放旧脏内容。现有 v1 草稿检查点明确拒绝带 lineage 的候选，防止丢失历史后误报持久化；不能据此宣称重启后的产品 Undo 已完成。
