...
HAE-010 第一阶段已验证覆盖前的私有准备：Main 固定原始身份/版本、合作实例锁、备份/候选/关联记录经独占创建与回读，重新检查时区分基线/候选/冲突并保留遗留锁。`prepared` 不代表 HTML 已保存；没有执行下面的临时替换、committed 或基线更新，合同见 [保存准备](SAVE_PREPARATION.md)。
