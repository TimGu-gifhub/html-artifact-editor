...
HAE-011 第七阶段已实现 [纯核心历史与来源重建](HISTORY.md)，包含 prepare/commit、独立保存点、分支与私有逻辑记录校验；真实 Worker、Windows 事务和 Chromium 重开分别验证。尚未接入 Workspace 历史命令、空 Text Preview 安装或完整历史存储。现有 v1 草稿检查点明确拒绝带 lineage 的候选，防止丢失历史后误报持久化；不能据此宣称重启后的产品 Undo 已完成。
