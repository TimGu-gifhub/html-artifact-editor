...
HAE-011 已实现 [逻辑草稿检查点](DRAFT_CHECKPOINTS.md) 的纯核心重建、Main 私有存储和可选 Apply 队列。恢复输入只有逻辑 Text 意图；从匹配的原始基线重新解析节点和上下文，生成新范围/编码，核对完整候选 hash。当前授权文件身份与 hash 不符即拒绝；与保存 committed 的精确新版本对应时拒绝重复应用。最新点按会话修订选择，不能越过较新不完整或归零点；明确结束标记阻止旧会话恢复和继续写入。两类记录共用目录、锁和配额，不自动删除证据。窗口离开与结束标记协调、恢复窗口、清理、历史和遗留锁处理仍待接入。
