...
版本：schema 1 设计基线。HAE-003/004 已实现静态索引与纯字节 Patch 候选；HAE-005 验证 Main 草稿和独占新文件另存；HAE-010 验证 Windows 覆盖事务及窗口会话的新基线。正常产品接线、跨保存的历史与恢复仍待实现。
...
HAE-011 第六阶段完成第 5 步的数据与保存确认路径：[源码 Diff](SOURCE_DIFF.md) 使用同一冻结基线/候选的真实 UTF-8 切片，完整呈现实体和换行等词法变化。范围、内容和候选 hash 均由 Main 复核；从 Diff 发起的 Save 携带所显示的修订/hash，过期确认在事务开始前拒绝。保存仍通过 HAE-010 备份、版本冲突和显式提交，新基线重新建索引；产品 Diff 面板与跨保存反向编辑尚未实现。
