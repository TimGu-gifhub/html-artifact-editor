...
版本：schema 1 设计基线。HAE-003/004 已实现静态索引与纯字节 Patch 候选；HAE-005 验证 Main 草稿和独占新文件另存；HAE-010 验证 Windows 覆盖事务及窗口会话的新基线。正常产品接线、窗口历史命令与完整历史恢复仍待实现。
...
HAE-011 已实现 [逻辑草稿检查点](DRAFT_CHECKPOINTS.md) 的纯核心重建、Main 私有存储和可选 Apply 队列。恢复输入只有逻辑 Text 意图；从匹配的原始基线重新解析节点和上下文，生成新范围/编码，核对完整候选 hash。当前授权文件身份与 hash 不符即拒绝；与保存 committed 的精确新版本对应时拒绝重复应用。最新点按会话修订选择，不能越过较新不完整或归零点；明确结束标记阻止旧会话恢复和继续写入。窗口离开已在输入冻结、写入排空及视图试挂载后核验结束标记；失败/未知保留原草稿及源证据并阻止继续操作。两类记录共用目录、锁和配额，不自动删除证据。可信 Workspace 已能在重新授权、核验最新记录和源版本后，将候选安装到新 Preview 映射；恢复不写 HTML，不重复持久化同一修订。产品恢复控件、故障恢复操作、清理、历史和遗留锁处理仍待接入。
...
HAE-011 第七阶段已实现 [纯核心历史与来源重建](HISTORY.md)，包含 prepare/commit、独立保存点、分支与私有逻辑记录校验；真实 Worker、Windows 事务和 Chromium 重开分别验证。第八阶段将新来源证明接入隔离 Preview：先核对实际 DOM，按明确的空 Text 安装合同创建缺席对象，再按映射版本与旧值确认单 Text 历史操作。Workspace 历史命令、输入协调与完整历史存储仍未接入。现有 v1 草稿检查点明确拒绝带 lineage 的候选，防止丢失历史后误报持久化；不能据此宣称重启后的产品 Undo 已完成。
