...
原生关闭复用 [文档与窗口生命周期](WORKSPACE_LIFECYCLE.md)，同一 current 上处理取消、放弃或明确另存后关闭。普通可选端口实验在覆盖保存进行中即时拒绝关闭；离开确认尚未提供覆盖保存选项，不开放绕过恢复的强制关闭命令。

持久化工厂现已固定启用 [退出协调](APPLICATION_QUIT.md) 的保存等待与清理屏障：加入已开始的 Save/备份恢复，确认其成功且无清理警告后再检查关闭；失败或未知结果保留窗口。Main requestClose 与原生 close 共用一个结果，批准后先完成 runtime.dispose 再销毁窗口。旧可选端口实验仍采用即时拒绝保存忙碌状态的规则。

Main dispose 的重复调用共用同一完成结果。关闭连接后，等待 Workspace 已开始的准备/事务与候选清理、当前文档关闭，以及包含旧重连在内的 bridge 命令排空；未知结果和失败清理继续保留引用。持久化工厂只在所有清理及存储空闲校验通过后释放进程内实例占用，不把窗口已消失视为工作已结束；应用退出事件由 Main 安装的协调器加入同一结果。
