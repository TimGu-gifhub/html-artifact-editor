...
Main dispose 的重复调用共用同一完成结果。关闭连接后，等待 Workspace 已开始的准备/事务与候选清理、当前文档关闭，以及包含旧重连在内的 bridge 命令排空；未知结果和失败清理继续保留引用。持久化工厂只在所有清理及存储空闲校验通过后释放进程内实例占用，不把窗口已消失视为工作已结束；应用退出协调仍由 Main 调用者负责。
