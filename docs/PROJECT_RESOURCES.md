...
日期：2026-09-10。Main 的目录授权、入口切换和诊断已接入统一窗口会话，并提供原生选择器适配器及只读开发入口。产品目录打开、入口切换及诊断面板已接入；[模式切换](MODE_SWITCH.md) 在同一根授权内重建静态校稿或脚本只读文档。人工对话框与维护者验收仍待完成，执行记录见 [HAE-008](implementation/HAE-008.md)。
