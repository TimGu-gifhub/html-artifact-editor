...
日期：2026-09-10；HAE-005 第七阶段。Main 可把应用退出与持久化窗口的同一关闭决定、文件事务和清理结果连通。真实 Electron 进程实验已执行；HAE-009 正常产品入口已安装此服务，并加入当前输入窗口的排空和已知辅助窗口清理；原生确认人工操作、真实 IME 与 OS 关机仍待验收。
...
[产品中断检查](INTERRUPTION_WORKFLOW.md) 复用同一退出协调：等待已接受的核验/确认/处置，再排空输入并进入 Workspace 关闭；处理结果未知或收尾有警告时保留窗口。已进入关闭流程时不能再开始新检查。确认提交后的 renderer 丢失不撤销处置，所有已接受的 bridge 命令仍由下述屏障加入。
