...
Main transport 的 close 同步撤销权限和 handlers，drain 仅等待此前接受的命令结算。统一窗口 teardown 同时等待已关闭的旧连接，不会因页面消失就提前报告文件工作完成；命令成功或失败都不恢复旧页面权限。drain 不是 renderer 方法，接口字段和写入授权不变。
