...
日期：2026-09-10；HAE-011 第十三阶段。已实现纯核心逻辑编辑意图、Main 私有检查点存储、窗口持久化端口，以及最新点选择、会话结束标记、窗口离开协调、恢复安装、精确提交后的历史重建、旧点有界清理及清理中断后的显式恢复，执行记录见 [HAE-011](implementation/HAE-011.md)。底层存储只接受 Main 调用；正常 Windows 工作台已接通受限恢复、历史和 Diff 控件，并提供文件/项目目录的重新授权选项。检查点成功仅表示该版本的私有证据完成写入和回读，不表示 HTML 已保存。故障处置产品入口、跨会话清理与人工验收仍待完成。
...
[窗口备份恢复](BACKUP_RESTORE.md) 属于单独确认的 HTML 替换，先冻结输入、排空队列并核验最新干净修订/hash，不能跨过尚未持久化的归零点。成功后使用新文件版本和新的干净历史，旧检查点仍作为证据保留；它不会延续旧 checkpointSessionId 或回放旧 Text 意图。
...
写入沿用持有文件句柄、sync、完整回读 hash 和目录身份复核；最后再次加载并重建候选。记录不依赖前一检查点，写入失败不覆盖旧点；已被更新完整点替代的 v2 记录按第 3 节清理。文件 sync 与进程强杀测试不证明断电后目录元数据的持久性。
...
每目标合计最多 20 份草稿/保存记录，整个目录合计 200 MiB，根最多枚举 512 项。保存目录最多七个已知文件，v1 草稿目录最多四个，v2 最多五个；origin.bin 计入相同配额。配额在同一锁内计算，包含不完整和已结束会话的证据。结束标记追加到既有目录，不占新记录名额，但仍需 2 KiB 的容量余量。

Main 当前持有文档所有权的 v2 序列按 [检查点清理合同](CHECKPOINT_COMPACTION.md) 保留最近两个无歧义完整修订，删除更旧的完整点；全部逻辑历史包含在保留点内。不完整/损坏/歧义点、v1、其他会话、结束锚点和所有保存备份继续保留。先写新点再清理；已有序列满额时，可在新写入前用其最近两个完整点释放旧点容量。根下最多 64 KiB 的 compaction.json 记录精确删除意图并计入配额；未确认完成的清理保留锁和仍存在的记录，catalog 要求检查，不自动续删或解锁。其他证据或完整历史本身仍可能耗尽配额，达到限制明确失败；尚未完成跨会话/备份/故障证据清理和长期使用验收。

清理中断后可按 [显式恢复合同](COMPACTION_RECOVERY.md) 处理：Main 重新授权源文件，取得 profile 与私有目录独占审查，固定两个完整保留点和原锁/记录。取消只读，确认后先追加恢复记录及完成标记，再移除原 journal 与精确旧锁。完整恢复记录保留并计入字节/枚举配额；部分或损坏记录阻止普通写入/恢复。活跃文档须先完成关闭，此接口不解冻仍持有内存输入的窗口，也不处理一般 Save/结束事务旧锁。
...
Main 给 [createWorkspaceSession](../src/main/workspace/session.ts) 提供同目录的 checkpoints 端口后，每份校稿文档创建独立 [持久化队列](../src/main/draft/persistence.ts)，固定打开时 SaveSource、SourceIndex 和 checkpointSessionId。普通打开时它等于文档会话 ID；恢复时沿用原持久化序列，UI 文档身份独立更新。正常 Windows 产品入口已安装此端口；没有端口的实验或脚本只读模式下 current.persistence=null。
...
结束操作保留原 record/baseline/seal 和全部 HTML 字节，不释放记录配额。后续清理必须先处理该会话其他记录，不能先移除结束标记及其锚点而留下旧点，否则会丢失会话已结束的证据。当前有界清理仅针对仍活跃的同一 v2 序列，不删除结束锚点，不处理遗留锁或跨会话记录。
...
正常 Windows 产品入口、恢复控件及历史/Diff 面板已接入，见 [产品工作台](LIVE_WORKBENCH.md) 和 [源码 Diff](SOURCE_DIFF.md)。结束失败/未知时保留的冻结会话目前不能由用户在产品中继续处理；没有强制离开、自动解锁或绕过标记的重试。尚未预览的输入只可能保留在输入窗口或 Main 内存中；没有持久化它，也没有持久化未获 Preview 确认的 uncertainCandidate。活动历史计算已使用有限 Worker；私有存储验证与 saved 历史准备仍由 Main 调用纯核心，满配额/大历史的交互性能尚需验证。精确提交后的完整历史恢复已接通，但新干净点失败、遗留锁或更高修订不完整时仍需要后续明确的故障处理，不能绕过证据。

产品恢复每次默认文件方式，只授权 HTML 所在文件夹。用户可改选项目目录，经现有 restore 的 sourceMode='directory' 让 Main 重新选择根和入口；不会从记录名重建路径或沿用旧授权。恢复前排空真实输入窗口，固定记录/来源方式/文档身份并复查最新修订；组词、失败输入、忙碌、清理和待复核结果不能触发恢复。对话框同步锁定本次操作，取消或失败保留选项，记录及操作结果均绑定当前打开代次。部分或歧义记录不会因此变得可恢复。

证据使用自制文件，覆盖完整候选字节、外部冲突、记录损坏、写入异常、实际进程强杀与原生保存去重。[产品验收](implementation/HAE-012.md) 还覆盖子目录报告、父级共享 CSS、两次 Main 终止后重新授权、键盘选择、取消/错误与重复操作，以及复核保存和独立 Edge 重开。真实 IME、原生选择器、维护者报告、Windows 10/macOS、真实磁盘满或断电仍未验收；HAE-011 和 M2 保持未完成。
