...
日期：2026-09-10；HAE-011 第十二阶段。已实现纯核心逻辑编辑意图、Main 私有检查点存储、可选窗口持久化端口，以及最新点选择、会话结束标记、窗口离开协调、恢复安装、精确提交后的历史重建、旧点有界清理及清理中断后的显式恢复，执行记录见 [HAE-011](implementation/HAE-011.md)。调用者须为 Main；正常产品入口与恢复界面尚未接入。检查点成功仅表示该版本的私有证据完成写入和回读，不表示 HTML 已保存。
