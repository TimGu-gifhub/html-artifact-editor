...
HAE-011 的 [草稿检查点](DRAFT_CHECKPOINTS.md) 共用同一私有目录和 active.lock；其 record.json/baseline.bin/complete.json 及可选 retired.json 独立 schema 也计入上述总配额，结束标记不释放记录名额。保存 scan 跳过可归类的草稿目录；混合文件、未知文件名或无法验证归属的记录头阻止新的准备，损坏证据仍计入配额。Main 通过目录身份链防止将不同存储目录的提交证据混配。现有普通 v1 和恢复 v2 记录保持可读，提交阶段与替换协议未变。
