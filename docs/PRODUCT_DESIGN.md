...
状态：2026-09-10 已选方案 B，HAE-009 接通 Windows 产品工作台。实时预览、勾选复核、隐藏/独立浮窗和 PDF 的当前合同见 [产品工作台](LIVE_WORKBENCH.md)，执行证据见 [HAE-009](implementation/HAE-009.md)。下述未纳入该记录的设计目标仍待实现或实机验收；原型的五处演示文字限制不适用于产品。
...
| 区域 | 常驻内容 | 行为 |
| --- | --- | --- |
| 顶部 | 文件名、打开、撤销/重做、变更数、复核保存、PDF | 保存按钮始终明确；窗口标题用未保存标志；产品入口使用静态校稿 |
| 中央预览 | 原页面与选择高亮 | 浏览滚动不被截断；不覆盖整页 contenteditable |
| 右侧校稿栏 | 原文、纯文本输入、预览状态、还原本段、只读原因 | 输入短暂停顿自动预览；可隐藏、拆为独立浮窗及收回 |
| 变更面板 | 修改列表、原文/新文、逐条/全选复核、源码 Diff | 全部当前净变更复核后才能保存；再改某条取消该条复核 |
...
HAE-007 已用同一任务和自制报告展示预览优先、校稿侧栏优先、变更复核优先三种视觉方案，维护者已选择方案 B，作为颜色、间距、字号、焦点样式和组件的基础。设计稿至少包含正常编辑、只读提示、未保存、冲突和窄窗口状态。
...
flowchart LR
    ...
    Select --> Draft[编辑草稿]
    Draft -->|短暂停顿自动预览| Dirty[待保存变更]
    Draft -->|取消待预览输入| Select
    Dirty --> Review[逐条或全选复核]
    Review --> Diff[检查最新源码变化]
    Diff --> Save[保存并备份]
    Save -->|成功| Clean[已保存]
...
| 自动预览草稿 | 输入停顿约 250 毫秒后串行校验当前文档/选择版本，确认后更新预览与历史 | 不写入 HTML 文件；组词期间不提交 |
| 取消/Escape | 取消尚未预览输入，保留已确认草稿；已预览内容用还原本段或撤销恢复 | 源 HTML 零写入；还原已预览内容形成可撤销记录 |
| 失焦 | 排空当前输入并更新草稿；组合态或失败保留输入并解释原因 | 不自动保存 HTML、不丢失失败文本 |
| 编辑时点击另一段、切换文件或模式 | 先处理当前待预览输入；组词或失败保持当前目标 | 不把输入应用给新目标；模式切换先处理草稿再运行脚本，规则见 [模式合同](MODE_SWITCH.md) |
...
早期输入位于可信侧栏。2026-09-11 后续切片已将正常入口改为可信原位输入：在所选 Text 位置按源显示字体输入，当前面板在浏览/编辑及核验保存后延续；工具栏可直接复核和全选保存。侧栏与浮窗仍可选。几何暂不可用时保留同一输入元素；真实 IME/多屏验收仍独立待执行。

2026-09-11 已授权加入可选就地小窗：参考“浏览/编辑切换、精确文字标记、附近编辑”的交互，使用可信原生小窗及独立鼠标穿透装饰窗，保持纯文字输入。新绑定自动聚焦、实时预览、组词拒绝转移和复核保存已有自动产品证据；真实 IME、原生拖动/鼠标穿透和多屏 DPI 仍待人工验收。此更新不表示上表全部设计目标已实现，详见 [就地校稿](CONTEXTUAL_EDIT.md)。

## 4. 中文输入、粘贴与键盘

- `compositionstart` 到 `compositionend` 期间不应用、不切换目标、不响应应用级 Enter/Escape/Ctrl+S。Escape 首先交给输入法；组合结束后再响应明确的新按键，不能重放旧保存指令。
- Enter 在多行输入框插入文本换行；Ctrl+Enter / Cmd+Enter 立即排空当前输入。默认无需点击应用。输入中的 Tab 按焦点顺序移动，不插入不可见字符。
...
输入预览局部更新 Text。实际保存后重新建立受控预览与源码映射；经完整源码核验的面板和文档滚动已可延续，选择须重新建立，不能复用旧 offset。交互预览需要重新载入时，提示动态运行状态会重置。保存后恢复选中片段的逻辑位置，若无法重新定位则清空选择。
...
“清理本地记录”沿用方案 B 菜单和有名称/焦点约束的对话框，只在打开文档前检查全部私有草稿、历史和备份。UI 显示范围和当前 Main 状态；最终清单及永久删除确认在 Main 原生对话框中完成，默认取消。检查/确认/清理期间锁定对话框及其他操作；取消或失败保持结果可见，unknown/收尾警告不提供直接重试。关闭后焦点回到稳定的“更多操作”按钮，窄窗保留说明和按钮。续做数量是上次确认的原清单，不能表述为当前剩余数。完整状态与删除限制见 [本地记录清理](RECORD_CLEANUP.md)。

- 干净状态直接关闭。
- 先向当前输入窗口请求排空；组词、断连或失败保留窗口。有未保存草稿时提供“取消 / 另存草稿后继续 / 放弃修改”。如需覆盖原 HTML，取消关闭后完成工作台复核保存。
- 保存失败留在原窗口；不能因为已点击关闭而销毁草稿。
- 正在保存时禁止取消到一半或并发退出；完成后按结果继续关闭或保留窗口。
- 崩溃重启展示可恢复草稿的文件名、保存时间和基线状态；只有 hash 与文件身份匹配时才允许用户恢复，默认不自动写盘。
- 每次确认的自动预览后异步持久化恢复记录；显示最后持久化状态。尚未预览的本地字符不能承诺在崩溃或掉电后恢复。浮窗关闭表示收回校稿栏，仍由主窗口拥有文档和退出决定。
