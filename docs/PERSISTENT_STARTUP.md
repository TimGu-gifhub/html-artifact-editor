...
产品另装配 [中断检查控制器](INTERRUPTION_WORKFLOW.md)。原生关闭先加入已经接受的维护操作，再排空实际输入窗口；bridge 的 drain 与 disposeAuxiliary 共同等待同一结果。未知结果或收尾警告使控制器拒绝正常关闭/清理，运行实例及 profile 所有权继续保留。它不改变下述固定目录与持久化清理顺序。
