...
HAE-011 core history now provides prepared/confirmed transitions, logical Undo/Redo, savepoint rebasing and strict private record round-trips. Read docs/HISTORY.md before extending it. SourceIndex.lineage is Main/private history data, not a renderer capability. Reparse both sources and verify the full tree and every literal source gap before reconstructing an emptied Text at a fresh zero-length range; never accept an old offset or strip lineage to bypass verification. Draft/Diff workers rebuild this proof. Main must confirm Preview application before history commit, and verify the actual saved version before rebasing. Unknown outcomes retain old history and candidate evidence. Workspace history commands, empty Text installation in the isolated Preview registry and durable full-history storage remain unimplemented; do not pass an augmented tree directly to the existing registry or claim v1 checkpoint persistence is sufficient. v1 checkpoints reject lineage with DRAFT_HISTORY_UNSUPPORTED before disk writes. `npm run test:history` verifies core/worker/native-save integration, not product UI or durable history recovery.
