...
This is HTML Artifact Editor, a local-first visual text correction tool for existing HTML. HAE-001/002 add the toolchain and read-only project previews; HAE-003/004 verify static mapping and byte patches. HAE-005 experiments connect drafts, isolated Text changes, exclusive copies, pending input and a scoped trusted-editor IPC bridge. The normal app still has no product editing UI, overwrite saving or recovery. Read README.md, PRD.md, ARCHITECTURE.md, docs/PATCH_SPEC.md and the target Issue before implementation.
