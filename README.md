...
第七阶段已实现 [逻辑历史与保存后的来源重建](docs/HISTORY.md)：准备/确认操作组、撤销/重做分支和保存点；清空后保存的 Text 通过新基线的完整结构与字节证明重新定位。真实 Worker、Windows 事务和 Chromium 重开覆盖该计算路径。Workspace 历史命令、空 Text 预览安装及完整历史磁盘格式尚未接入，当前产品仍不能操作这些历史能力。
