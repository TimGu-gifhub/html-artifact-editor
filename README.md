...
第六阶段将 [备份恢复接入统一窗口](docs/BACKUP_RESTORE.md)：可信接口只列出当前文件的备份摘要，单独确认并处理输入/持久化保护后恢复；成功核验文件与新映射，建立干净历史并可再次编辑保存。取消、版本变化和未知结果均保留相应输入或证据。正常产品备份列表与确认界面仍待接入。

HAE-005 第六阶段补齐 [Main 持久化启动装配](docs/PERSISTENT_STARTUP.md)：同一固定私有目录统一提供保存、草稿和备份服务，排除重复实例，并在窗口销毁时等待操作/清理结束。真实进程重启可发现已持久化草稿；五处文字校稿、Diff、保存、重开 Undo 和备份还原已在同一装配入口验证。正常应用仍未安装该服务，产品界面与退出流程尚待接入。
