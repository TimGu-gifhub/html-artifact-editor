...
HAE-010 已连接 [保存事务与窗口会话实验](docs/implementation/HAE-010.md)：私有备份/候选、合作实例锁、Windows 原生替换、结果回读与提交记录，经可信 IPC 显式保存后重新解析并建立新基线。失败或未知结果保留草稿与证据，重启支持只读检查。尚未接入正常应用，跨保存的撤销历史、持久化草稿、恢复向导和产品验收仍待完成。
