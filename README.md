...
HAE-011 已实现 [私有草稿检查点与会话持久化实验](docs/implementation/HAE-011.md)：确认变化的 Apply 通过有界队列异步写入，准确报告已持久化版本，失败保留草稿并等待显式重试；Save 与关闭协调队列结束。Main 按修订选择最新点，支持明确结束会话的持久标记；冲突、损坏、较新不完整点或净变更归零时不会自动恢复旧脏点。恢复重新核验来源，精确提交版本阻止重复应用。正常入口、窗口离开与结束标记接线、恢复到窗口、清理、撤销/重做及源码 Diff 尚未完成。
