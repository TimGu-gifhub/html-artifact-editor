...
HAE-011 第一阶段实现 [私有草稿检查点](docs/implementation/HAE-011.md)：保存逻辑文字意图和原始基线，恢复候选时重新核验来源并生成字节位置；草稿与备份共用锁和配额，保存后的精确提交版本可识别并拒绝重复应用。每次 Apply 的自动持久化、恢复到窗口、撤销/重做及源码 Diff 尚未接入。
